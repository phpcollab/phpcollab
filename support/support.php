...
try {
    ...
    if ($supportType == "team") {
        if ($action == "new") {
            $listRequests = $support->getSupportRequestByStatusAndProjectId(0, $projectId, $block1->sortingValue);
        } elseif ($action == "open") {
            $listRequests = $support->getSupportRequestByStatusAndProjectId(1, $projectId, $block1->sortingValue);
        } elseif ($action == "complete") {
            $listRequests = $support->getSupportRequestByStatusAndProjectId(2, $projectId, $block1->sortingValue);
        }
    } elseif ($supportType == "admin") {
    ...
    if ($listRequests) {
        ...
        foreach ($listRequests as $listItem) {
            ...
            $block1->cellRow($blockPage->buildLink(
                "../support/viewrequest.php?id=" . $listItem["sr_id"],
                $escaper->escapeHtml($listItem["sr_subject"]),
                "in"
            ));
        ...
        if ($teamMember == "true" || $session->get("profile") == "0") {
            ...
            $block1->paletteScript(2, "remove",
                "../support/deleterequests.php?project=" . $projectId ."&sendto=" . $request->query->get('action') . "&action=deleteRequest", "false,true,true",
                $strings["delete"]);
