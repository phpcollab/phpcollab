...
try {
    ...
    $block1->contentRow($strings["name"], $escaper->escapeHtml($subtaskDetail['subtas_name']));

    //$block1->contentRow($strings["description"], nl2br($subtaskDetail['subtas_description']));
    $block1->contentRow($strings["description"], nl2br($escaper->escapeHtml($subtaskDetail['subtas_description'])));
    ...
    if (
        !empty($subtaskDetail['subtas_complete_date'])
    ...
    $block1->contentRow($strings["comments"], nl2br($escaper->escapeHtml($subtaskDetail['subtas_comments'])));
    ...
    $assignmentHistoryBlock = new phpCollab\Block();
    $assignmentHistoryBlock->sorting("assignment", $sortingUser["assignment"], "ass.assigned DESC",
        $sortingFields = array(0 => "ass.comments", 1 => "mem1.login", 2 => "mem2.login", 3 => "ass.assigned"));
    $assignmentList = $assignments->getAssignmentsBySubtaskId($id, $assignmentHistoryBlock->sortingValue);

    if ($assignmentList) {

        $assignmentHistoryBlock->form = "ahT";
        $assignmentHistoryBlock->openForm("../subtasks/viewsubtask.php?&id=$id&task=$task#" . $assignmentHistoryBlock->form . "Anchor", null, $csrfHandler);

        $assignmentHistoryBlock->headingToggle($strings["assignment_history"], $request->cookies->get( $assignmentHistoryBlock->form ));

        $assignmentHistoryBlock->openResults($checkbox = "false");

        $assignmentHistoryBlock->labels($labels = array(
            0 => $strings["comment"],
        ...
        foreach ($assignmentList as $assignment) {
            $assignmentHistoryBlock->openRow();
            $assignmentHistoryBlock->checkboxRow($assignment["ass_id"], $checkbox = "false");
            if ($assignment["ass_comments"] != "") {
                $assignmentHistoryBlock->cellRow($assignment["ass_comments"]);
            } elseif ($assignment["ass_assigned_to"] == "0") {
                $assignmentHistoryBlock->cellRow($strings["task_unassigned"]);
            } else {
                $assignmentHistoryBlock->cellRow($strings["task_assigned"] . " " . $assignment["ass_mem2_name"] . " (" . $assignment["ass_mem2_login"] . ")");
            }
            $assignmentHistoryBlock->cellRow($blockPage->buildLink($assignment["ass_mem1_email_work"], $assignment["ass_mem1_login"], "mail"));
            if ($assignment["ass_assigned_to"] == "0") {
                $assignmentHistoryBlock->cellRow($strings["unassigned"]);
            } else {
                $assignmentHistoryBlock->cellRow($blockPage->buildLink($assignment["ass_mem2_email_work"], $assignment["ass_mem2_login"],
                    "mail"));
            }
            $assignmentHistoryBlock->cellRow(phpCollab\Util::createDate($assignment["ass_assigned"], $session->get("timezone")));
            $assignmentHistoryBlock->closeRow();
        }
        $assignmentHistoryBlock->closeResults();

        $assignmentHistoryBlock->closeToggle();
        $assignmentHistoryBlock->closeFormResults();
    }
