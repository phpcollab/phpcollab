            ...
            try {
                if ($csrfHandler->isValid($request->request->get("csrf_token"))) {
                    ...
                    if ($request->request->get("action") == "add") {
                        ...
                        if ($taskStatus == "1") {
                            $subtasks->setCompletionDate($newSubtask["subtas_id"], $date);
                        }
                        ...
                        if ($assignedTo != "0") {
                            $assignments->assignSubtask($newSubtask["subtas_id"], $session->get("id"), $assignedTo, $timestamp);

                            //if assigned_to is not blank, set assigned date
                            $subtasks->setAssignedDate($newSubtask["subtas_id"], $timestamp);

                            //if assigned_to is set, add to team members (only if they are not a team member already)
                            ...
                            if ($notifications == "true") {
                                try {
                                    $subtasks->sendNotification("assignment", $newSubtask, $projectDetail, $session,
                                        $logger);
                                } catch (Exception $exception) {
                        ...
                        phpCollab\Util::headerFunction("../subtasks/viewsubtask.php?id={$newSubtask["subtas_id"]}&task=$parentTaskId&msg=add");
                    }
                }
            } catch (InvalidCsrfTokenException $csrfTokenException) {
                $logger->error('CSRF Token Error', [
...
HTML;
    ...
    $teamMembers = $teams->getTeamByProjectId($project, null, null, 'mem.name');
